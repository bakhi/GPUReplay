		...
		u64 *gpu_va)
{
   ...
   /**  EE("Allocating %lld va_pages, %lld commit_pages, %lld extent, 0x%llX flags", */
		/** va_pages, commit_pages, extent, *flags);	// jin */
	...
	if (*flags & BASE_MEM_SAME_VA) {
		VV("[jin] KBASE_REG_ZONE_SAME_VA");
		rbtree = &kctx->reg_rbtree_same;
		zone = KBASE_REG_ZONE_SAME_VA;
	} else if ((*flags & BASE_MEM_PROT_GPU_EX) && kbase_has_exec_va_zone(kctx)) {
		VV("[jin] KBASE_REG_ZONE_EXEC_VA");
		rbtree = &kctx->reg_rbtree_exec;
		zone = KBASE_REG_ZONE_EXEC_VA;
	} else {
		VV("[jin] KBASE_REG_ZONE_CUSTOM_VA");
		rbtree = &kctx->reg_rbtree_custom;
		zone = KBASE_REG_ZONE_CUSTOM_VA;
	}

	// jin: allocate object but is not part of any list yet, 
	// and is flagged as KBASE_REG_FREE. NO mapping is allocated yet.

	// jin: memory flag
	/** EE("[jin] mem flags : %x", *flags); */
	/** dump_stack(); */
   /**  EE("Allocating %lld va_pages, %lld commit_pages, %lld extent, 0x%llX flags", */
		/** va_pages, commit_pages, extent, *flags);	// jin */
	VV("[jin] kbase_alloc_free_region gpu_va: 0x%llx", *gpu_va);
	...
	// jin: update region flags based on the flags comes from user space
	...
	/***************************/
	// jin: allocate pages here
	/***************************/
	// jin: # the size of the requested region, in pages, and # of commited pages 
	WW("requested region size (in pages): %lu, commited pages: %lu", va_pages, commit_pages);
	...
	if (reg->flags & KBASE_REG_PERMANENT_KERNEL_MAPPING) {
		...
		VV("[jin] -------- PERMANENT_KERNEL_MAPPING");
		if (err < 0) {
			kbase_gpu_vm_unlock(kctx);
	...
	if (*flags & BASE_MEM_SAME_VA) {	// jin: default - fall here
		...
		/* return a cookie */
		// jin: cookies is bitmask containing of BITS_PER_LONG bits, used mainly for SAME_VA allocations
		// to defer the reservation of memory region (from the GPU virtual address space)
		// from base_mem_alloc_ioctl to mmap system call. This helps returning unique handles, disguised
		// as GPU VA, to Userspace from base_mem_alloc and later retrieving the pointer to memory region structure
		// in the mmap handler
		...
		*gpu_va = (u64) cookie;
		VV("gpu_va = (u64) cookie: 0x%llx", (unsigned long long) *gpu_va);
	} else /* we control the VA */ {
		VV("[jin] ------ kbase_gpu_mmap -----------1");
...
bool kbase_mem_evictable_unmake(struct kbase_mem_phy_alloc *gpu_alloc)
{
	...
	if (gpu_alloc->evicted == 0) {
		...
		if (gpu_alloc->reg) {
			 ...
			 */
			if (!err) {
				WW("invoke kbase_mem_grow_gpu_mapping");
		...
		struct kbase_va_region *reg)
{
	...
	WW("invoke kbase_mmu_insert_pages");
...
int kbase_mem_commit(struct kbase_context *kctx, u64 gpu_addr, u64 new_pages)
{
	...
	WW("gpu_addr: %llx, new_pages: %llu, reg->nr_pages: %llu, reg->gpu_alloc->nents: %llu", \
		gpu_addr, new_pages, reg->nr_pages, reg->gpu_alloc->nents);
...
static void kbase_cpu_vm_open(struct vm_area_struct *vma)
{
	...
	KBASE_TRACE_ADD(kbdev, JIN_KBASE_VM_OPS_OPEN, NULL, NULL, 0u, 0);
	KBASE_DEBUG_ASSERT(map);
	KBASE_DEBUG_ASSERT(map->count > 0);
	/* non-atomic as we're under Linux' mm lock */
	map->count++;
	KBASE_TRACE_ADD(kbdev, JIN_KBASE_VM_OPS_DONE, NULL, NULL, 0u, 0 /* open = 0 */);
}
...
static void kbase_cpu_vm_close(struct vm_area_struct *vma)
{
	...
	KBASE_TRACE_ADD(kbdev, JIN_KBASE_VM_OPS_CLOSE, NULL, NULL, 0u, 0);
	...
	if (map->free_on_close) {
		...
		if (!(current->flags & PF_EXITING)) {
			WW("invoke kbase mem free region. start pfn: %llx, nr_pages: %lu", \
				map->region->start_pfn, map->region->nr_pages);
	...
	kfree(map);
	KBASE_TRACE_ADD(kbdev, JIN_KBASE_VM_OPS_DONE, NULL, NULL, 0u, JIN_KBASE_TRACE_IDX(JIN_KBASE_VM_OPS_CLOSE, JIN_KBASE_VM_OPS_OPEN));
}
...
static vm_fault_t kbase_cpu_vm_fault(struct vm_fault *vmf)
{
	...
	struct kbase_device *kbdev = map->kctx->kbdev;
#ifdef CONFIG_GR
	struct gr_context *gr_ctx = map->kctx->gr_ctx;
#endif
	...
	VV("[jin] kbase_cpu_vm_fault");
	KBASE_TRACE_ADD(kbdev, JIN_KBASE_VM_OPS_FAULT, NULL, NULL, vma->vm_start, 0);
	...
	if (unlikely(map->region->cpu_alloc->type == KBASE_MEM_TYPE_ALIAS)) {
		...
		/** EE("nents = aliased->length = %lu start paddr: %llx", nents, as_phys_addr_t(pages[0])); */
	} else  {
		nents = map->alloc->nents;
		pages = map->alloc->pages;
		/** EE("nents = map->alloc->nents = %lu start paddr: %llx", nents, as_phys_addr_t(pages[0])); */
	}

	fault_pgoff = map_start_pgoff + (vmf->pgoff - vma->vm_pgoff);
	/** WW("vm_start : %llx, vm_end : %llx, vm_pgoff : %llx, region->start_pfn : %llx", vma->vm_start, vma->vm_end, vma->vm_pgoff, map->region->start_pfn); */
	WW("vm_start: %llx, vm_end: %llx, nents:%lu, paddr[0]: %llx, f_pgoff: %lu", vma->vm_start, vma->vm_end, nents, as_phys_addr_t(pages[0]), fault_pgoff);
...
#ifdef CONFIG_GR
	if (gr_ctx) {
//		EE("kctx : %p, gr_ctx : %p", (void *)map->kctx, (void *)gr_ctx);
		gr_as_add_valid(gr_ctx, vma, nents);
	}
#endif
...
/** #ifdef CONFIG_GR */
#if 0
	// jin: in/output region has both CPU and GPU CACHED properties
	if (gr_ctx) {
		if (map->alloc->reg && (map->alloc->reg->flags & KBASE_REG_CPU_CACHED) && (map->alloc->reg->flags & KBASE_REG_GPU_CACHED)) {
			EE("[Fault] vm_start: %llx vm_end: %llx nents: %lu", \
				vma->vm_start, vma->vm_end, nents);
			if (nents == 513) {
				// jin : print last page only
				EE("---- cur va : %llx pa: %llx", vma->vm_start + PAGE_SIZE * 512, pages[512]);
				hex_dump((void *)(vma->vm_start + PAGE_SIZE * 512), 64);
			} else {
				int k;
				for (k = 0; k < min(nents, 4UL); k++) {
					EE("---- cur va : %llx pa: %llx", vma->vm_start + PAGE_SIZE * k, pages[k]);
					hex_dump((void *)(vma->vm_start + PAGE_SIZE * k), 64);
...
exit:
	kbase_gpu_vm_unlock(map->kctx);
	KBASE_TRACE_ADD(kbdev, JIN_KBASE_VM_OPS_DONE, NULL, NULL, vma->vm_start, JIN_KBASE_TRACE_IDX(JIN_KBASE_VM_OPS_FAULT, JIN_KBASE_VM_OPS_OPEN));
	return ret;
}
...

{
	...
	*aligned_offset = 0;
	...
	VV("vma->vm_pgoff = %lu", vma->vm_pgoff);
	VV("kbasep_reg_mmap: cookie : %d", cookie);
	reg = kctx->pending_regions[cookie];	// jin guess: get the region allocate before
	if (!reg) {
		err = -ENOMEM;
		goto out;
	}
	...
	*nr_pages = kbase_reg_current_backed_size(reg);
	...
	// jin guess: cpu vm address comes in
   /**  EE("[jin] kbase_gpu_mmap ------4	gr_ctx : %p", (void *) kctx->gr_ctx); */
	/** EE("[jin] kbase_gpu_mmap ------4    start_pfn : %llx", reg->start_pfn); */
	/** EE("[jin] kbase_gpu_mmap ------4    nr_pagse: %lu, vma->vm_start : 0x%lx", *nr_pages, vma->vm_start); */
	/** EE("[jin] kbase_gpu_mmap ------4    aligned_offset = %lu", *aligned_offset); */
	...
	VV("reg->start_pfn : %llx", reg->start_pfn);
...
#ifdef CONFIG_GR
	if (gr_ctx) {
		gr_as_add(gr_ctx, vma, reg);
	}
#endif
	VV("[jin] kbase_context_mmap DONE");
	VV("flags : %08x", reg->flags);
		...
		struct kbase_vmap_struct *map, enum kbase_sync_type dest)
{
	...
	/** WW("cpa_pa :%llx, offset: %lu, size: %lu", cpu_pa, offset, map->size); */
		...
		struct kbase_vmap_struct *map)
{
	...
	WW("page_count : %lu", page_count);
	for (i = 0; i < page_count; i++) {
		pages[i] = as_page(page_array[page_index + i]);
		WW("page[%d]: 0x%llx", i, pages[i]);
	}
