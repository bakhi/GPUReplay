		...
		struct tagged_addr *pages, bool partial_allowed)
{
	...
	VV("[jin] ---- alloc_pages(4k=%zu):", nr_4k_pages);
	VV("[jin] ---- alloc_pages(internal=%zu):", nr_pages_internal);

	/* Get pages from this pool */
	kbase_mem_pool_lock(pool);
	nr_from_pool = min(nr_pages_internal, kbase_mem_pool_size(pool));
	VV("[jin] nr_pages_internal: %lu, kbase_mem_pool_size(pool): %lu", \
		nr_pages_internal, kbase_mem_pool_size(pool));
	VV("[jin] nr_from_pool : %lu", nr_from_pool);
	while (nr_from_pool--) {
		int j;
		p = kbase_mem_pool_remove_locked(pool);
		if (pool->order) {
	...
	if (i != nr_4k_pages && pool->next_pool) {
		...
		VV("[jin] Get any remaining pages from kernel");
		while (i != nr_4k_pages) {
			/** VV("[jin] --- (Get any remaining pages from kernel) invoke kbase_mem_alloc_page"); */
			p = kbase_mem_alloc_page(pool);
			if (!p) {
				if (partial_allowed)
					goto done;
		...
		struct tagged_addr *pages, bool dirty, bool reclaimed)
{
	...
	VV("---------- kbase mem pool free pages (%zu)", nr_pages);
	...
	VV("---------- kbase mem pool free pages nr to pool (%zu)", nr_to_pool);
	...
	VV("--------- free_pages(%zu) done nr to kernel", nr_pages - nr_to_pool);
