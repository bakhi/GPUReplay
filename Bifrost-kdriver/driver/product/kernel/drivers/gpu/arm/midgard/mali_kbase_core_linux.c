...
#include <linux/delay.h>	// jin: for delay
#endif
			       ...
			       struct kbase_ioctl_version_check *version)
{
	...
	KBASE_TRACE_ADD(kfile->kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_VERSION_CHECK, JIN_KBASE_IOCTL_VERSION_CHECK));
	return err;
}
	...
	base_context_create_flags const flags)
{
	...
	if (IS_ERR_OR_NULL(kctx->kctx_dentry)) {
		...
		VV("[jin] kbase_context_debugfs_init");
...
static int kbase_open(struct inode *inode, struct file *filp)
{
	...
	KBASE_TRACE_ADD(kbdev, JIN_KBASE_FOPS_OPEN, NULL, NULL, 0u, 0);
	kfile = kbase_file_new(kbdev, filp);

	KBASE_TRACE_ADD(kbdev, JIN_KBASE_FOPS_DONE, NULL, NULL, 0u, 0); // xzl
	...
	//KBASE_TRACE_ADD(kbdev, JIN_KBASE_FOPS_DONE, NULL, NULL, 0u, 0);
	// xzl: moved up otherwise the branch below uncovered
	// NB: can't trace after kbase_release_device() anyway
	return 0;
...
static int kbase_release(struct inode *inode, struct file *filp)
{
	...
	KBASE_TRACE_ADD(kfile->kbdev, JIN_KBASE_FOPS_RELEASE, NULL, NULL, 0u,
			JIN_KBASE_TRACE_IDX(JIN_KBASE_FOPS_RELEASE, JIN_KBASE_FOPS_OPEN));

	kbase_file_delete(kfile);
	// jin: cannot add trace after kbdev is released
//	KBASE_TRACE_ADD(kfile->kbdev, JIN_KBASE_FOPS_DONE, NULL, NULL, 0u, 0);
	return 0;
}
		...
		struct kbase_ioctl_set_flags *flags)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_SET_FLAGS, JIN_KBASE_IOCTL_VERSION_CHECK));
	return err;
}
...
{
	int ret = 0;
	/** k2_measure("kbase_api_job_submit"); */
	/** EE("[jin]------------[kbase_api_job_submit], addr: 0x%llx, nr_atoms: %u", submit->addr, submit->nr_atoms);	// jin */

#ifdef CONFIG_GR
	if (kctx->gr_ctx) {
		gr_dump_mc_valid(kctx->gr_ctx, GR_AS_TYPE_ALL_WO_COMPUTE_JOB);
		gr_print_as(kctx->gr_ctx, 0);

		/** gr_mmu_dump(kctx->gr_ctx, kctx); */
	}
#endif
	ret  = kbase_jd_submit(kctx, u64_to_user_ptr(submit->addr),
			submit->nr_atoms,
			submit->stride, false);
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_JOB_SUBMIT, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
		...
		struct kbase_ioctl_get_gpuprops *get_props)
{
	...
	if (get_props->flags != 0) {
		...
		KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
			JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_GET_GPUPROPS, JIN_KBASE_IOCTL_VERSION_CHECK));
		return -EINVAL;
	}
   ...
   /**  VV("[jin] buffer : %p userbuf: %p\n", get_props->buffer, u64_to_user_ptr(get_props->buffer)); */
	/** VV("[jin] phys mem: %p\n", virt_to_phys(u64_to_user_ptr(get_props->buffer))); */
	/** VV("[jin] kprop->prop_buffer : %p", kprops->prop_buffer); */
...
{
	JJ("end of gpu execution");
#ifdef CONFIG_GR
/** #ifdef CONFIG_GR */
	if (kctx->gr_ctx) {
		// jin: dump mmu here
		// XXX Issue: if we dump pgt too late, the entry flags is set as inavlid
		// This requires post-process from replayer
		gr_mmu_dump(kctx->gr_ctx, kctx);	// XXX kctx is now member of gr_ctx

		// jin: we separately dump COMPUTE_JOB address space
		// reason: during run time, the user-space runtime adds more compute jobs into memory region
		// problem: additional job is written to the same page. If we dump it eariler, there might be missed job inthe dumpted page
		// additional problem: inside compute jobs, the valid bit is set as invalid by GPU? or runtime? when done.
		// rw, we just handle it from replayer side by scanning all the compute jobs and set all of them as valid
		gr_dump_mc_valid(kctx->gr_ctx, GR_AS_TYPE_COMPUTE_JOB);
		gr_dump_mc_invalid(kctx->gr_ctx, GR_AS_TYPE_ALL);
		gr_print_as(kctx->gr_ctx, 1);
		gr_dump_sync_as(kctx->gr_ctx);
	}
#endif
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
			JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_POST_TERM, JIN_KBASE_IOCTL_VERSION_CHECK));
	return 0;
}
		...
		union kbase_ioctl_mem_alloc *alloc)
{
	...
	reg = kbase_mem_alloc(kctx, alloc->in.va_pages,
			...
			alloc->in.extent,
			&flags, &gpu_va);	// jin: gpu_va is a kind of handler used for mapping later

	if (!reg)
		return -ENOMEM;
	VV("va_pages: %llu, commit_pages: %llu", alloc->in.va_pages, alloc->in.commit_pages);
	VV("out_va: 0x%llx", (unsigned long long) gpu_va);

	alloc->out.flags = flags;
	alloc->out.gpu_va = gpu_va;

	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_MEM_ALLOC, JIN_KBASE_IOCTL_VERSION_CHECK));
	return 0;
}
...
{
	int ret = kbase_mem_query(kctx, query->in.gpu_addr,
			query->in.query, &query->out.value);
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_MEM_QUERY, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
...
{
	int ret = kbase_mem_free(kctx, free->gpu_addr);
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_MEM_FREE, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
...
{
	int ret = kbase_vinstr_hwcnt_reader_setup(kctx->kbdev->vinstr_ctx, setup);
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_HWCNT_SET, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
		...
		struct kbase_ioctl_hwcnt_enable *enable)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_HWCNT_ENABLE, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
...
static int kbase_api_hwcnt_dump(struct kbase_context *kctx)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_HWCNT_DUMP, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
...
static int kbase_api_hwcnt_clear(struct kbase_context *kctx)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_HWCNT_CLEAR, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
		...
		union kbase_ioctl_get_cpu_gpu_timeinfo *timeinfo)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_GET_CPU_GPU_TIMEINFO, JIN_KBASE_IOCTL_VERSION_CHECK));
	return 0;
}
		...
		struct kbase_ioctl_disjoint_query *query)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_DISJOINT_QUERY, JIN_KBASE_IOCTL_VERSION_CHECK));
	return 0;
}
		...
		struct kbase_ioctl_get_ddk_version *version)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_GET_DDK_VERSION, JIN_KBASE_IOCTL_VERSION_CHECK));
	return len;
}
...
{
	int ret = kbase_region_tracker_init_exec(kctx, exec_init->va_pages);
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_MEM_EXEC_INIT, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
		...
		struct kbase_ioctl_mem_sync *sync)
{
	...
	WW("============================ [MEM_SYNC] type: %u, gpu_addr: %llx sync_size: %llu", \
		sync->type, sync->handle, sync->size);
	ret = kbase_sync_now(kctx, &sset);
	/* jin: handle = gpu addr
	 *		type = cpu to gpu or gpu to cpu
	 *		size = sync size
	 */		
	/** KBASE_TRACE_ADD_SLOT_INFO(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, sync->handle, sync->type, sync->size); */
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, sync->handle, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_MEM_SYNC, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
...
{
	int ret = kbasep_find_enclosing_cpu_mapping_offset(
			kctx,
			find->in.cpu_addr,
			find->in.size,
			&find->out.offset);
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_MEM_FIND_CPU_OFFSET, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
...
{
	int ret = kbasep_find_enclosing_gpu_mapping_start_and_offset(
			kctx,
			find->in.gpu_addr,
			find->in.size,
			&find->out.start,
			&find->out.offset);
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_MEM_FIND_GPU_START_AND_OFFSET, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
		...
		struct kbase_ioctl_get_context_id *info)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_GET_CONTEXT_ID, JIN_KBASE_IOCTL_VERSION_CHECK));
	return 0;
}
...
{
	int ret = kbase_timeline_io_acquire(kctx->kbdev, acquire->flags);
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_TLSTREAM_ACQUIRE, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
...
static int kbase_api_tlstream_flush(struct kbase_context *kctx)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_TLSTREAM_FLUSH, JIN_KBASE_IOCTL_VERSION_CHECK));
	return 0;
}
...
{
	int ret = kbase_mem_commit(kctx, commit->gpu_addr, commit->pages);
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_MEM_COMMIT, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
		...
		union kbase_ioctl_mem_alias *alias)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_MEM_ALIAS, JIN_KBASE_IOCTL_VERSION_CHECK));
	return 0;
}
		...
		union kbase_ioctl_mem_import *import)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_MEM_IMPORT, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
		...
		struct kbase_ioctl_mem_flags_change *change)
{
	...
	ret = kbase_mem_flags_change(kctx, change->gpu_va,
			change->flags, change->mask);
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_MEM_FLAGS_CHANGE, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
...
{
#if defined(CONFIG_SYNC) || defined(CONFIG_SYNC_FILE)
	int ret = kbase_sync_fence_validate(validate->fd);
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_FENCE_VALIDATE, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
#else
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_FENCE_VALIDATE, JIN_KBASE_IOCTL_VERSION_CHECK));
	return -ENOENT;
#endif
}
		...
		struct kbase_ioctl_mem_profile_add *data)
{
	...
	ret = kbasep_mem_profile_debugfs_insert(kctx, buf, data->len);
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_MEM_PROFILE_ADD, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
		...
		struct kbase_ioctl_soft_event_update *update)
{
	...
	ret = kbase_soft_event_update(kctx, update->event, update->new_status);
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_SOFT_EVENT_UPDATE, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
		...
		struct kbase_ioctl_sticky_resource_map *map)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_STICKY_RESOURCE_MAP, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
		...
		struct kbase_ioctl_sticky_resource_unmap *unmap)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_STICKY_RESOURCE_UNMAP, JIN_KBASE_IOCTL_VERSION_CHECK));
	return ret;
}
		...
		struct kbase_ioctl_tlstream_test *test)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_TLSTREAM_TEST, JIN_KBASE_IOCTL_VERSION_CHECK));
	return 0;
}
		...
		struct kbase_ioctl_tlstream_stats *stats)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, \
		JIN_KBASE_TRACE_IDX(JIN_KBASE_IOCTL_TLSTREAM_STATS, JIN_KBASE_IOCTL_VERSION_CHECK));
	return 0;
}
...
static long kbase_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case KBASE_IOCTL_VERSION_CHECK:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_VERSION_CHECK, NULL, NULL, 0u, (unsigned long) cmd);
	...
	case KBASE_IOCTL_SET_FLAGS:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_SET_FLAGS, NULL, NULL, 0u, (unsigned long) cmd);
	...
	case KBASE_IOCTL_JOB_SUBMIT:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_JOB_SUBMIT, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_JOB_SUBMIT");
		VV("----------- uarg %p arg %lu", uarg, arg);
		/** k2_measure("job submit"); */
	...
	case KBASE_IOCTL_GET_GPUPROPS:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_GET_GPUPROPS, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_GET_GPUPROPS");
	...
	case KBASE_IOCTL_POST_TERM:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_POST_TERM, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_POST_TERM");
		KBASE_HANDLE_IOCTL(KBASE_IOCTL_POST_TERM,
				kbase_api_post_term,
				kctx);
		break;
	case KBASE_IOCTL_MEM_ALLOC:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_MEM_ALLOC, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_MEM_ALLOC");
	...
	case KBASE_IOCTL_MEM_QUERY:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_MEM_QUERY, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_MEM_QUERY");
	...
	case KBASE_IOCTL_MEM_FREE:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_MEM_FREE, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_MEM_FREE");
	...
	case KBASE_IOCTL_DISJOINT_QUERY:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_DISJOINT_QUERY, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_DISJOINT_QUERY");
	...
	case KBASE_IOCTL_GET_DDK_VERSION:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_GET_DDK_VERSION, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_GET_DDK_VERSION");
	...
	case KBASE_IOCTL_MEM_JIT_INIT_OLD:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_MEM_JIT_INIT_OLD, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_MEM_JIT_INIT_OLD");
	...
	case KBASE_IOCTL_MEM_JIT_INIT:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_MEM_JIT_INIT, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_MEM_JIT_INIT");
	...
	case KBASE_IOCTL_MEM_EXEC_INIT:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_MEM_EXEC_INIT, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_MEM_EXEC_INIT");
	...
	case KBASE_IOCTL_MEM_SYNC:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_MEM_SYNC, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_MEM_SYNC");
	...
	case KBASE_IOCTL_MEM_FIND_CPU_OFFSET:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_MEM_FIND_CPU_OFFSET, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_MEM_FIND_CPU_OFFSET");
	...
	case KBASE_IOCTL_MEM_FIND_GPU_START_AND_OFFSET:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_MEM_FIND_GPU_START_AND_OFFSET, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_MEM_FIND_GPU_START_AND_OFFSET");
	...
	case KBASE_IOCTL_GET_CONTEXT_ID:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_GET_CONTEXT_ID, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_GET_CONTEXT_ID");
	...
	case KBASE_IOCTL_TLSTREAM_ACQUIRE:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_TLSTREAM_ACQUIRE, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_TLSTREAM_ACQUIRE");
	...
	case KBASE_IOCTL_TLSTREAM_FLUSH:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_TLSTREAM_FLUSH, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_TLSTREAM_FLUSH");
		KBASE_HANDLE_IOCTL(KBASE_IOCTL_TLSTREAM_FLUSH,
				kbase_api_tlstream_flush,
				kctx);
		break;
	case KBASE_IOCTL_MEM_COMMIT:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_MEM_COMMIT, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_MEM_COMMIT");
	...
	case KBASE_IOCTL_MEM_ALIAS:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_MEM_ALIAS, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_MEM_ALIAS");
	...
	case KBASE_IOCTL_MEM_IMPORT:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_MEM_IMPORT, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_MEM_IMPORT");
	...
	case KBASE_IOCTL_MEM_FLAGS_CHANGE:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_MEM_FLAGS_CHANGE, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_MEM_FLAGS_CHANGE");
	...
	case KBASE_IOCTL_STREAM_CREATE:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_STREAM_CREATE, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_STREAM_CREATE");
	...
	case KBASE_IOCTL_FENCE_VALIDATE:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_FENCE_VALIDATE, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_FENCE_VALIDATE");
	...
	case KBASE_IOCTL_MEM_PROFILE_ADD:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_MEM_PROFILE_ADD, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_MEM_PROFILE_ADD");
	...
	case KBASE_IOCTL_SOFT_EVENT_UPDATE:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_SOFT_EVENT_UPDATE, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_SOFT_EVENT_UPDATE");
	...
	case KBASE_IOCTL_STICKY_RESOURCE_MAP:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_STICKY_RESOURCE_MAP, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_STICKY_RESOURCE_MAP");
	...
	case KBASE_IOCTL_STICKY_RESOURCE_UNMAP:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_STICKY_RESOURCE_UNMAP, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_STICKY_RESOURCE_UNMAP");
	...
	case KBASE_IOCTL_HWCNT_READER_SETUP:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_HWCNT_READER_SETUP, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_HWCNT_READER_SETUP");
	...
	case KBASE_IOCTL_HWCNT_ENABLE:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_HWCNT_ENABLE, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_HWCNT_ENABLE");
	...
	case KBASE_IOCTL_HWCNT_DUMP:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_HWCNT_DUMP, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_HWCNT_DUMP");
		KBASE_HANDLE_IOCTL(KBASE_IOCTL_HWCNT_DUMP,
				kbase_api_hwcnt_dump,
				kctx);
		break;
	case KBASE_IOCTL_HWCNT_CLEAR:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_HWCNT_CLEAR, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_HWCNT_CLEAR");
		KBASE_HANDLE_IOCTL(KBASE_IOCTL_HWCNT_CLEAR,
				kbase_api_hwcnt_clear,
				kctx);
		break;
	case KBASE_IOCTL_GET_CPU_GPU_TIMEINFO:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_GET_CPU_GPU_TIMEINFO, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_GET_CPU_GPU_TIMEINFO");
	...
	case KBASE_IOCTL_HWCNT_SET:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_HWCNT_SET, NULL, NULL, 0u, (unsigned long) cmd);
		KBASE_HANDLE_IOCTL_IN(KBASE_IOCTL_HWCNT_SET,
		VV("[jin] kbase_ioctl KBASE_HANDLE_IOCTL_IN");
				kbase_api_hwcnt_set,
	...
	case KBASE_IOCTL_CINSTR_GWT_START:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_CINSTR_GWT_START, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_CINSTR_GWT_START");
		KBASE_HANDLE_IOCTL(KBASE_IOCTL_CINSTR_GWT_START,
				kbase_gpu_gwt_start,
				kctx);
		break;
	case KBASE_IOCTL_CINSTR_GWT_STOP:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_CINSTR_GWT_STOP, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_CINSTR_GWT_STOP");
		KBASE_HANDLE_IOCTL(KBASE_IOCTL_CINSTR_GWT_STOP,
				kbase_gpu_gwt_stop,
				kctx);
		break;
	case KBASE_IOCTL_CINSTR_GWT_DUMP:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_CINSTR_GWT_DUMP, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_CINSTR_GWT_DUMP");
	...
	case KBASE_IOCTL_TLSTREAM_TEST:
		KBASE_TRACE_ADD(kbdev, JIN_KBASE_IOCTL_TLSTREAM_TEST, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_TLSTREAM_TEST");
	...
	case KBASE_IOCTL_TLSTREAM_STATS:
		KBASE_TRACE_ADD(kbdev, KBASE_IOCTL_TLSTREAM_STATS, NULL, NULL, 0u, (unsigned long) cmd);
		VV("[jin] kbase_ioctl KBASE_IOCTL_TLSTREAM_STATS");
...
static ssize_t kbase_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	//KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_FOPS_KBASE_READ, NULL, NULL, 0u, 0);
	//xzl: moved below
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_FOPS_READ, NULL, NULL, 0u, 0);
	...
	do {
		while (kbase_event_dequeue(kctx, &uevent)) {
			...
			if (filp->f_flags & O_NONBLOCK) {
				KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_FOPS_DONE, NULL, NULL, 0u,
							JIN_KBASE_TRACE_IDX(JIN_KBASE_FOPS_READ, JIN_KBASE_FOPS_OPEN));
				return -EAGAIN;
			}
 ...
 out:
  //xzl: TODO cover all exits above
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_FOPS_DONE, NULL, NULL, 0u,
			JIN_KBASE_TRACE_IDX(JIN_KBASE_FOPS_READ, JIN_KBASE_FOPS_OPEN));
...
static unsigned int kbase_poll(struct file *filp, poll_table *wait)
{
	...
	// xzl: moved here
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_FOPS_POLL, NULL, NULL, 0u, 0);

	poll_wait(filp, &kctx->event_queue, wait);
	if (kbase_event_pending(kctx)) {
		KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_FOPS_DONE, NULL, NULL, 0u,
				JIN_KBASE_TRACE_IDX(JIN_KBASE_FOPS_POLL, JIN_KBASE_FOPS_OPEN));
...
static int kbase_mmap(struct file *const filp, struct vm_area_struct *const vma)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_FOPS_MMAP, NULL, NULL, vma->vm_start, 0);
	if (unlikely(!kctx))
		return -EPERM;

	VV("[jin]: kbase_mmap is invoked");
	ret = kbase_context_mmap(kctx, vma);
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_FOPS_DONE, NULL, NULL, vma->vm_start,
			JIN_KBASE_TRACE_IDX(JIN_KBASE_FOPS_MMAP, JIN_KBASE_FOPS_OPEN));
	return ret;
}
		...
		const unsigned long pgoff, const unsigned long flags)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_FOPS_GET_UNMAPPED_AREA, NULL, NULL, 0u, 0);
	ret = kbase_context_get_unmapped_area(kctx, addr, len, pgoff, flags);
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_FOPS_DONE, NULL, NULL, ret,
			JIN_KBASE_TRACE_IDX(JIN_KBASE_FOPS_GET_UNMAPPED_AREA, JIN_KBASE_FOPS_OPEN));
	return ret;
}
...
static int kbase_common_reg_map(struct kbase_device *kbdev)
{
	...
	EE("[%s] kbdev->reg_start: 0x%llx kbdev->reg: %p size : %lu", \
		__func__, kbdev->reg_start, kbdev->reg, kbdev->reg_size);
...
int registers_map(struct kbase_device * const kbdev)
{
	...
	kbdev->reg_snapshot = vzalloc(kbdev->reg_size * kbdev->max_reg_snapshots);
#endif

	/** EE("[%s]REG_START : 0x%llx", __func__, kbdev->reg_start); */
...
int kbase_device_debugfs_init(struct kbase_device *kbdev)
{
	...
	dev_info(kbdev->dev, "[jin] kbase_device_debugfs_init\n");
...
#ifdef CONFIG_DEBUG_FS
	EE("[DEFINE] CONFIG_DEBUG_FS");
#endif

#ifdef CONFIG_MALI_MIDGARD_ENABLE_TRACE
	EE("[DEFINE] CONFIG_MALI_MIDGARD_ENABLE_TRACE");
#endif

#ifdef CONFIG_MALI_SYSTEM_TRACE
	EE("[DEFINE] CONFIG_MALI_SYSTEM_TRACE");
#endif

#ifdef CONFIG_MALI_GATOR_SUPPORT
	EE("[DEFINE] CONFIG_MALI_GATOR_SUPPORT");
#endif

#ifdef CONFIG_MALI_DEBUG
	EE("[DEFINE] CONFIG_MALI_DEBUG");
#endif

#ifdef CONFIG_MALI_MIDGARD_DVFS
	EE("[DEFINE] CONFIG_MALI_MIDGARD_DVFS");
#endif


/******************** PM related ***************************/
#ifdef CONFIG_PM_RUNTIME
	EE("[DEFINE] CONFIG_PM_RUNTIME");
#endif

#ifdef CONFIG_PM
	EE("[DEFINE] CONFIG_PM");
	EE("linux version code : %x", LINUX_VERSION_CODE);
	EE("kernel version : %x", KERNEL_VERSION(3, 19, 0));
	#ifdef KBASE_PM_RUNTIME
		EE("KBASE_PM_RUNTIME : %d", KBASE_PM_RUNTIME);
	#endif
#endif
/**********************************************************/
/******************** configuration ***********************/
	EE("SLOT_RB_SIZE: %d", (int) SLOT_RB_SIZE);
/**********************************************************/


#if KBASE_TRACE_ENABLE
	EE("[DEFINE] KBASE_TRACE_ENABLE = %d", (int) KBASE_TRACE_ENABLE);
...
static int kbase_platform_device_probe(struct platform_device *pdev)
{
	...
	gr_io_history_init(&kbdev->gr_io_history, GR_DEFAULT_REGISTER_HISTORY_SIZE);
	kbdev->gr_io_file 			= file_open("/home/linaro/gr_io_history", O_WRONLY | O_CREAT, 0644);
	kbdev->gr_io_file_offset 	= 0;
#endif

#ifdef CONFIG_GR_REG_SNAPSHOT
	kbdev->reg_dump_file		= file_open("/home/linaro/gr_reg_snapshot", O_WRONLY | O_CREAT, 0644);
	kbdev->reg_dump_file_offset	= 0;
	kbdev->reg_dump_cnt			= 0;
	kbdev->max_reg_snapshots	= 500;
	kbdev->nr_reg_snapshots		= 0;
	JJ("open reg snapshot file");
#endif
...
static int kbase_device_suspend(struct device *dev)
{
	...
	WW("################# device suspend ###################");
	KBASE_TRACE_ADD(kbdev, JIN_KBASE_PM_OPS_DEVICE_SUSPEND, NULL, NULL, 0u, 0);
	...
	KBASE_TRACE_ADD(kbdev, JIN_KBASE_PM_OPS_DONE, NULL, NULL, 0u, 0);
	return 0;
}
...
static int kbase_device_resume(struct device *dev)
{
	...
	KBASE_TRACE_ADD(kbdev, JIN_KBASE_PM_OPS_DEVICE_RESUME, NULL, NULL, 0u, 0);
	...
	KBASE_TRACE_ADD(kbdev, JIN_KBASE_PM_OPS_DONE, NULL, NULL, 0u, JIN_KBASE_TRACE_IDX(JIN_KBASE_PM_OPS_DEVICE_RESUME, JIN_KBASE_PM_OPS_DEVICE_SUSPEND));
	return 0;
}
...
static int kbase_device_runtime_suspend(struct device *dev)
{
	...
	KBASE_TRACE_ADD(kbdev, JIN_KBASE_PM_OPS_DEVICE_RUNTIME_SUSPEND, NULL, NULL, 0u, 0);
	WW("########### runtime_suspend ###########");
	dev_dbg(dev, "Callback %s\n", __func__);
#if defined(CONFIG_MALI_DEVFREQ) && \
		(LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0))
	...
	KBASE_TRACE_ADD(kbdev, JIN_KBASE_PM_OPS_DONE, NULL, NULL, 0u, JIN_KBASE_TRACE_IDX(JIN_KBASE_PM_OPS_DEVICE_RUNTIME_SUSPEND, JIN_KBASE_PM_OPS_DEVICE_SUSPEND));
	return 0;
}
...
static int kbase_device_runtime_resume(struct device *dev)
{
	...
	KBASE_TRACE_ADD(kbdev, JIN_KBASE_PM_OPS_DEVICE_RUNTIME_RESUME, NULL, NULL, 0u, 0);
	dev_dbg(dev, "Callback %s\n", __func__);
	...
	KBASE_TRACE_ADD(kbdev, JIN_KBASE_PM_OPS_DONE, NULL, NULL, 0u, JIN_KBASE_TRACE_IDX(JIN_KBASE_PM_OPS_DEVICE_RUNTIME_RESUME, JIN_KBASE_PM_OPS_DEVICE_SUSPEND));
	return ret;
}
#endif /* KBASE_PM_RUNTIME */
