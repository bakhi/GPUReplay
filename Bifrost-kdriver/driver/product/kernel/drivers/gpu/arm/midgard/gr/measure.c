...
static int job = 0;	/* # of submitted job */
static int job_irq = 0; /* # of job irq */
static int gpu_irq = 0; /* # of gpu irq */
static int gpu_irq_reset = 0;
static int gpu_irq_clean_cache = 0;
static int gpu_irq_power_changed = 0;
static int mmu_irq = 0; /* # of gpu irq */
...
int k2_measure_clean(void)
{
	...
	gpu_irq = 0;
	gpu_irq_reset = 0;
	gpu_irq_clean_cache = 0;
	gpu_irq_power_changed = 0;
	mmu_irq = 0;
...
#error "unsupported arch"
	...
	for (j = start; j < end; j++) {
		...
		if (i == 0)
		  ...
		  if(strcmp(samples[i].msg, "job_submit_hw_start") == 0) job++;
		  else if(strcmp(samples[i].msg,"irq(JOB)") == 0) job_irq++;
		  else if(strcmp(samples[i].msg,"irq(GPU)") == 0) gpu_irq++;
		  else if(strcmp(samples[i].msg,"irq(RESET_COMPLETED") == 0) gpu_irq_reset++;
		  else if(strcmp(samples[i].msg,"irq(CLEAN_CACHES_COMPLETED)") == 0) gpu_irq_clean_cache++;
		  else if(strcmp(samples[i].msg,"irq(POWER_CHANGED_ALL)") == 0) gpu_irq_power_changed++;
		  else if(strcmp(samples[i].msg,"irq(MMU)") == 0) mmu_irq++;
...
#error "unsupported arch"
	...
	print_to_tracebuffer(" TOTAL: %u us \n",
			(samples[i].ts - samples[0].ts) / (CTM_FREQ / 1000 / 1000));
#elif defined (CONFIG_ARCH_OMAP4)
...
#error "unsupported arch"
	...
	print_to_tracebuffer("job(%d) job_irq(%d) gpu_irq(%d) mmu_irq(%d)", job, job_irq, gpu_irq, mmu_irq);
	print_to_tracebuffer("GPU_IRQ| reset(%d) clean_cache(%d) power_changed(%d)", \
		gpu_irq_reset, gpu_irq_clean_cache, gpu_irq_power_changed);
	print_to_tracebuffer("---------------------------------------------------------------\n");
