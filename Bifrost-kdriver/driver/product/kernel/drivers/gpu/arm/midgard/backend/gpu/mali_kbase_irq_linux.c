...
#ifdef CONFIG_GR
	u32 failed = val >> 16;
	u32 finished = (val & 0xFFFF) | failed;
	int js = ffs(finished) - 1;

	struct kbase_jd_atom *katom = kbase_gpu_inspect(kbdev, js, 0);
	struct gr_context *gr_ctx = katom->kctx->gr_ctx;
...
#ifdef JIN_MEASURE_IRQ 
	k2_measure("job_irq_test_handler");
#endif
	spin_lock_irqsave(&kbdev->hwaccess_lock, flags);
...
#ifdef JIN_MEASURE_IRQ
	// jin: add more test to measure job irq latency
	for (i = 0; i <50; i++) {
...
#ifndef CONFIG_GR
	// jin: turn off IRQ test for recording
	err = kbasep_common_test_interrupt(kbdev, JOB_IRQ_TAG);	// orig
	if (err) {
		dev_err(kbdev->dev, "Interrupt JOB_IRQ didn't reach CPU. Check interrupt assignments.\n");
		goto out;
	}
...
int kbase_install_interrupts(struct kbase_device *kbdev)
{
	...
	for (i = 0; i < nr; i++) {
		EE("[init IRQ] irq_num : %d, flags :%x dev_name: %s", kbdev->irqs[i].irq, kbdev->irqs[i].flags, dev_name(kbdev->dev));
