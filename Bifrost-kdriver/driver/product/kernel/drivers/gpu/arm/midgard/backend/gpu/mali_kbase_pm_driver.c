...
//#include <linux/time.h> //jin for time measurement
/****************************************************************/
...
static void mali_cci_flush_l2(struct kbase_device *kbdev)
{
	...
	EE("clean cache wait loops : %u", loops);
}
#endif
					...
					enum kbasep_pm_action action)
{
	...
	// jin: get hardare pm state based on core_type and action
	/** EE("reg : 0x%08x", reg); */
...
static u64 kbase_pm_l2_update_state(struct kbase_device *kbdev)
{
	...
	do {
		/* Get current state */
		u64 l2_trans = kbase_pm_get_trans_cores(kbdev,		// jin: L2_PWRTRANS_LO, HI
				KBASE_PM_CORE_L2);
		u64 l2_ready = kbase_pm_get_ready_cores(kbdev,		// jin: L2_READY_LO, HI
				KBASE_PM_CORE_L2);
		u64 tiler_trans = kbase_pm_get_trans_cores(kbdev,	// jin: TILER_PWRTRANS_LO, HI
				KBASE_PM_CORE_TILER);
		u64 tiler_ready = kbase_pm_get_ready_cores(kbdev,	// jin: TILER_READY_LO, HI
				KBASE_PM_CORE_TILER);
		...
		case KBASE_L2_OFF:	// jin: The L2 cache and tiler are off
			if (kbase_pm_is_l2_desired(kbdev)) {
				 ...
				 */
				/** EE("l2_present = %lu", l2_present);	// jin: Bifrost G71 has only one L2 cache */
				if (l2_present != 1)
		...
		case KBASE_L2_PEND_ON:	// jin: The L2 cache and tiler are powering on
		...
		case KBASE_L2_RESTORE_CLOCKS:	// jin: The GPU clock is restored. Conditionally used
		...
		case KBASE_L2_ON_HWCNT_ENABLE:	// jin: The L2 cache and tiler are on, and hwcnt is being enabled
		...
		case KBASE_L2_ON:	// jin: The L2 cache and tiler are on, and hwcnt is enabled
			if (!kbase_pm_is_l2_desired(kbdev)) {
		...
		case KBASE_L2_ON_HWCNT_DISABLE:	// jin: The L2 cache and tiler are on, and hwcnt is being disabled
		...
		case KBASE_L2_SLOW_DOWN_CLOCKS:	// jin: The GPU clock is set to appropriate or lowest clock. Conditionally used.
			/* We always assume only GPUs being affected by
			 * BASE_HW_ISSUE_GPU2017_1336 fall into this state
			 */
			WARN_ON_ONCE(!kbdev->pm.backend.gpu_clock_slow_down_wa);
		...
		case KBASE_L2_POWER_DOWN:	// jin: The L2 cache and tiler are about to be powered off
			if (!backend->l2_always_on)
				 ...
				 * powered off.
				 */
				/** EE("-- cache clean from L2 POWER DOWN (l2_always_on: %d)", backend->l2_always_on); */
				// jin: fall through in G71, l2 is always on.
		...
		case KBASE_L2_PEND_OFF:	// jin: The L2 cache and tiler are powering off
			if (!backend->l2_always_on) {
		...
		case KBASE_L2_RESET_WAIT:	// jin: The GPU is resetting, L2 cache and tiler power state are unknown
			/* Reset complete  */
			if (!backend->in_reset)
				backend->l2_state = KBASE_L2_OFF;
			break;
		...
		if (backend->l2_state != prev_state) 
			dev_dbg(kbdev->dev, "L2 state transition: %s to %s\n",
				...
				kbase_l2_core_state_to_string(
					backend->l2_state));
   /**      EE("L2 state transition: L2_%s to L2_%s", */
			/** kbase_l2_core_state_to_string(prev_state), */
			/** kbase_l2_core_state_to_string( */
				/** backend->l2_state)); */
...
static void kbase_pm_shaders_update_state(struct kbase_device *kbdev)
{
	...
	do {
		...
		// jin: G71 does not have corestack_driver_control by default
		switch (backend->shaders_state) {
		case KBASE_SHADERS_OFF_CORESTACK_OFF:	// jin: The shaders and core stacks are off
		...
		case KBASE_SHADERS_OFF_CORESTACK_PEND_ON:	// jin: The shaders are off, core stacks have been requested to power on and hwcnt is being disabled
			/** EE("--- Shaders off | corestack is pend on"); */
		...
		case KBASE_SHADERS_PEND_ON_CORESTACK_ON:	// jin: Core stacks are on, shaders have been requested to power on. Or after doing partial shader on/off, checking whether it's the desired state
		...
		case KBASE_SHADERS_ON_CORESTACK_ON:	// jin: The shaders and core stacks are on, and hwcnt already enabled.
			backend->shaders_avail = kbase_pm_ca_get_core_mask(kbdev);
		...
		case KBASE_SHADERS_ON_CORESTACK_ON_RECHECK:	// jin: The shaders and core stacks are on, hwcnt disabled, and checks to powering down or re-enabling hwcnt.
		...
		case KBASE_SHADERS_WAIT_OFF_CORESTACK_ON:	// jin: The shaders have been requested to power off, but they remain on for the duration of hte hysteresis timer
		...
		case KBASE_SHADERS_WAIT_GPU_IDLE:	// jin: The shaders partial poweroff needs to reach a state where jobs on the GPU are finished including jobs currently running and in the GPU queue because of GPU2017-861
		...
		case KBASE_SHADERS_WAIT_FINISHED_CORESTACK_ON:	// jin: The hysteresis timer has expired
			shader_poweroff_timer_queue_cancel(kbdev);

			if (kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_TTRX_921)) {
				/** EE("-- cache clean from SAHDER | corestack_driver_control = %d", corestack_driver_control); */
		...
		case KBASE_SHADERS_L2_FLUSHING_CORESTACK_ON:	// jin: The core stacks are on and the level 2 cache is being flushed
			if (!kbdev->cache_clean_in_progress)
				backend->shaders_state =
					KBASE_SHADERS_READY_OFF_CORESTACK_ON;
		...
		case KBASE_SHADERS_READY_OFF_CORESTACK_ON:	// jin: The core stacks are on and the shaders are ready to be powered off
		...
		case KBASE_SHADERS_PEND_OFF_CORESTACK_ON:	// jin: The core stacks are on, and the shader have been requested to power off
		...
		case KBASE_SHADERS_OFF_CORESTACK_PEND_OFF:	// jin: The shaders are off, and the core stacks have been requested to power off
		...
		case KBASE_SHADERS_OFF_CORESTACK_OFF_TIMER_PEND_OFF:	// jin: Shaders and corestacks are off, but the tick timer cancellation is till pending.
			if (!hrtimer_active(&stt->timer) && !stt->cancel_queued)
				backend->shaders_state = KBASE_SHADERS_OFF_CORESTACK_OFF;
			break;

		case KBASE_SHADERS_RESET_WAIT:	// jin: The GPU is resetting, shader and core stack power states are unknown
			/* Reset complete */
			if (!backend->in_reset)
				backend->shaders_state = KBASE_SHADERS_OFF_CORESTACK_OFF_TIMER_PEND_OFF;
			break;
		...
		if (backend->shaders_state != prev_state)
			dev_dbg(kbdev->dev, "Shader state transition: %s to %s\n",
				...
				kbase_shader_core_state_to_string(
					backend->shaders_state));
   /**      EE("Shader state transition: SHADERS_%s to SHADERS_%s", */
			/** kbase_shader_core_state_to_string(prev_state), */
			/** kbase_shader_core_state_to_string( */
				/** backend->shaders_state)); */
...
void kbase_pm_update_state(struct kbase_device *kbdev)
{
	...
	KBASE_TRACE_ADD(kbdev, JIN_PM_START, NULL, NULL, 0u, 0);
	if (!kbdev->pm.backend.gpu_powered) {
		EE("Do nothing if the GPU is off");
		return; /* Do nothing if the GPU is off */
	}

	/** dump_stack(); */

	// jin: pattern power_check1
	...
	// jin: pattern power_check2
	/** jin: if the current state matches the desired,
	  * the driver conducts unncessary checking which leads to redundant register read accesses */
	if (kbase_pm_is_in_desired_state_nolock(kbdev)) {
	...
	KBASE_TRACE_ADD(kbdev, JIN_PM_DONE, NULL, NULL, 0u, 0);
}
...
{
	EE("Power transition timed out unexpectedly");
...
void kbase_pm_clock_on(struct kbase_device *kbdev, bool is_resume)
{
	...
	spin_lock_irqsave(&kbdev->hwaccess_lock, flags);
	/** EE("[%s] kbase_ctx_sched_restore_all_as", __func__); */
...
static int kbase_pm_do_reset(struct kbase_device *kbdev)
{
	...
	/** jin: callback_soft_reset is optional callback to software reset the GPU.
	...
	if (kbdev->pm.backend.callback_soft_reset) {
		...
		II("[jin] CPU_COMMAND_SOFT_RESET");
		k2_measure("soft_reset_start");
#endif
...
#if (defined(CONFIG_GR_MEASURE_TIME) && CONFIG_GR_MEASURE_TIME & GR_PM_TIME)
	k2_measure("soft_reset_end");
	II("[jin] GPU_COMMAND_SOFT_RESET DONE");
#endif
