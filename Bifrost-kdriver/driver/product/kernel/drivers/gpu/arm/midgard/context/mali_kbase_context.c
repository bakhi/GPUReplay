...
void kbase_context_common_term(struct kbase_context *kctx)
{
	 ...
	 */
	VV("[jin] Flush the timeline stream, so the user can see the termination tracepoints being fired");
   /**  int enabled = 0; */
	/** enabled = atomic_read(&kctx->kbdev->timeline_is_enabled); */
	/** VV("[jin]: timeline_is_enabled: %d", enabled); */
...
int kbase_context_mem_alloc_page(struct kbase_context *kctx)
{
	...
	// jin: allocate one small page for the context?
	// it is not added to mem pool
	VV("[jin] === kbase_context_mem_alloc_page");
