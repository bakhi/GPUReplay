...
bool jd_submit_atom(struct kbase_context *kctx, const struct base_jd_atom_v2 *user_atom, struct kbase_jd_atom *katom)
{
	...
	/* Add dependencies */
	/** EE("[Add atom] current atom %d [%llx] core_req: %x", user_atom->atom_number, user_atom->jc, user_atom->core_req);	// jin */
	for (i = 0; i < 2; i++) {
		int dep_atom_number = user_atom->pre_dep[i].atom_id;
		...
		if (!dep_atom_number) {
			VV("[jin] no dep atom number, continued");
			continue;
		}
		VV("========================================");
   /**      EE("[Add dependency] current atom %d [%llx] depends on atom %d [%llx] number : %d dep type: %u", \ */
			/** user_atom->atom_number, user_atom->jc, dep_atom_number, dep_atom->jc, dep_atom_type); */
		VV("========================================");
	...
	if (katom->core_req & BASE_JD_REQ_EXTERNAL_RESOURCES) {
		VV("handle what we need to do to access the external resources"); // jin
	...
	if ((katom->core_req & BASE_JD_REQ_SOFT_JOB) == 0) {
		VV("katom->core_req != BASE_JD_REQ_SOFT_JOB");	// jin
	...
	if (katom->core_req & BASE_JD_REQ_SOFT_JOB) {
		...
		VV("[%s][%d] KBASE_JD_ATOM_STATE_IN_JS", __func__, __LINE__);	// jin
		katom->status = KBASE_JD_ATOM_STATE_IN_JS;
		...
		VV("This is a pure dependency. Resolve it immediately");
		...
		bool uk6_atom)
{
	...
	for (i = 0; i < nr_atoms; i++) {
		...
		// jin: user_addr is memory address of an array of struct base_jd_atom_v2
		...
		VV("[jin][%s] user_atom->jc: 0x%llx", __func__, user_atom.jc);
		VV("[jin] user_atom.extres_list: 0x%p", (void *) user_atom.extres_list);
		user_addr = (void __user *)((uintptr_t) user_addr + stride);	// jin: struct base_jd_atom_v2
...
#undef compiletime_assert_defined
#endif
		// jin: atoms are already stored ahead of time
		katom = &jctx->atoms[user_atom.atom_number];	// jin: atom_number is unique number to identify the atom
		...
		/** EE("atom %d need_to_try_schedule_context: %d", user_atom.atom_number, need_to_try_schedule_context); */
