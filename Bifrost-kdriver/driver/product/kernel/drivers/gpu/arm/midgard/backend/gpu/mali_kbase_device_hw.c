...
#ifdef CONFIG_GR_IO_HISTORY
	// jin: print offset instead of reg + offset to easily interpret it
	getrawmonotonic(&ts);
...
void kbase_reg_write(struct kbase_device *kbdev, u32 offset, u32 value)
{
...
#ifdef CONFIG_GR_REG_SNAPSHOT
	spin_lock_irqsave(&kbdev->reg_dump_lock, flags);
	writel(value, kbdev->reg + offset);
	gr_reg_dump(kbdev, 0, flags);
#else
	writel(value, kbdev->reg + offset);

#ifdef CONFIG_GR_IO_HISTORY
	// print offset instead of reg + offset to easily interpret it
	gr_io_history_add(&kbdev->gr_io_history, offset, value, 1);
#endif
	...
	if (unlikely(kbdev->io_history.enabled)) {
		...
		kbase_io_history_add(&kbdev->io_history, (void __iomem *) ((u64)offset),		// jin
		        value, 1);	// print offset instead of reg + offset to easily interpret it
#else
...
u32 kbase_reg_read(struct kbase_device *kbdev, u32 offset)
{
	...
	gr_reg_dump(kbdev, 1, flags);
#else
	val = readl(kbdev->reg + offset);

#ifdef CONFIG_GR_IO_HISTORY
	// print offset instead of reg + offset to easily interpret it
	gr_io_history_add(&kbdev->gr_io_history, offset, val, 0);
#endif
...
#ifdef CONFIG_GR_DEBUG
		kbase_io_history_add(&kbdev->io_history, (void __iomem *) ((u64)offset),		// jin
		        val, 0);	// print offset instead of reg + offset to easily interpret it
#else
		kbase_io_history_add(&kbdev->io_history, kbdev->reg + offset,
				val, 0);
#endif
