		...
		struct kbase_mmu_table *mmut)
{
	...
	// jin: kmap: map page addr to virtual addr and return vaddr
	page = kmap(p);	// jin: now page has kernel virtual addr corresponding to p
	if (page == NULL)
		goto alloc_free;
	...
	if (mmut->kctx) {
		int new_page_count;
#ifdef CONFIG_GR
		struct gr_context *gr_ctx = mmut->kctx->gr_ctx;
#endif
...
#ifdef CONFIG_GR
		// jin: count # of pages used for pgt
		atomic_inc(&gr_ctx->tmmu.pgt_nr_pages);
#endif
	}

	atomic_add(1, &kbdev->memdev.used_pages);

	// jin: KBASE_MMU_PAGE_ENTREIS = 512
	for (i = 0; i < KBASE_MMU_PAGE_ENTRIES; i++)
		kbdev->mmu_mode->entry_invalidate(&page[i]);	// jin: clear out or invalidate the pte
		...
		phys_addr_t *pgd, u64 vpfn, int level)
{
	...
	// |----------|----------|----------|----------|
	/** EE("----- level : %d  pgd : %llx-----", level, *pgd); */
	/** EE("vpfn0 : 0x%llx", vpfn); */
	vpfn >>= (3 - level) * 9;
	/** EE("vpfn1 : 0x%llx", vpfn); */
	vpfn &= 0x1FF;
	/** EE("vpfn2 : 0x%llx", vpfn); */
	...
	if (!target_pgd) {
		/** EE("no target_pgd"); */
		// jin: if page does not exist, allocate and get page for pgd
		target_pgd = kbase_mmu_alloc_pgd(kbdev, mmut);	
		if (!target_pgd) {
			dev_dbg(kbdev->dev, "%s: kbase_mmu_alloc_pgd failure\n",
					__func__);
			/** EE("%s: kbase_mmu_alloc_pgd failure\n", __func__); */
			kunmap(p);
			return -ENOMEM;
		}
#if 0
					...
					unsigned long flags, int const group_id)
{
	...
	VV("[jin] ------ kbase_mmu_insert_single_page");
	...
	while (remain) {
		...
		if (cur_level == MIDGARD_MMU_LEVEL(2)) {
			...
			*target = kbase_mmu_create_ate(kbdev, *phys, flags,
			...
			for (i = 0; i < count; i++) {
				// jin
			//    level 0    level 1   level 2    vindex
			//     9 bit      9 bit     9 bit      9 bit
			// |----------|----------|----------|----------|
				...
				// jin: set page table entry and return pte
				*target = kbase_mmu_create_ate(kbdev,
					phys[i], flags, cur_level, group_id);
				// jin: physical addrs and entry
				/** EE("ppa : 0x%llx and pte: 0x%llx", phys[i], *target); */
			}
		}
		...
		unsigned long flags, int as_nr, int const group_id)
{
	...
	WW("[%s] start vpfn: 0x%llx, nr: %u first phys addr:0x%llx", __func__, vpfn, nr, as_phys_addr_t(phys[0]));
		...
		u64 vpfn, size_t nr, bool sync)
{
	...
	// jin: op = AS_COMMAND_FLUSH_MEM
	VV("4");	// jin
	VV("[%s] op : %x", __func__, op);

//	dump_stack();
	VV("---- L2 cache flush start");	// jin
	err = kbase_mmu_hw_do_operation(kbdev,
			as, vpfn, nr, op, 0);
	VV("---- L2 cache flush done");	// jin
	...
	struct kbase_mmu_table *mmut, u64 vpfn, size_t nr, int as_nr)
{
	...
	/* jin: Dump mmu table before teardown some pages */
	/** gr_mmu_dump(mmut->kctx->gr_ctx, mmut->kctx); */
	/** WW("mmu_teardown_pages start_vpfn: 0x%llx, nr: %lu", vpfn, nr); */
					...
					unsigned long flags, int const group_id)
{
	...
	VV("kbase_mmu_update_pages_no_flush");
	...
	int const group_id)
{
	...
	while (!mmut->pgd) {
		...
		// jin: allocate pages and link them to device mem pool
		err = kbase_mem_pool_grow(
			&kbdev->mem_pools.small[mmut->group_id],
			MIDGARD_MMU_BOTTOMLEVEL);
		if (err) {
	...
	/** VV("[jin] ========== mmut->pgd : %pa[p]]\n", mmut->pgd); */
	/** EE("[jin] ========== mmut->pgd : %llx\n", mmut->pgd); */
#ifdef CONFIG_GR
	kctx->gr_ctx->tmmu.pgd = mmut->pgd;
#endif
	return 0;
}
		...
		int level, char ** const buffer, size_t *size_left)
{
	...
	if (*size_left >= size) {
		...
		memcpy(*buffer, (void *)&m_pgd, sizeof(m_pgd));
		*buffer += sizeof(m_pgd);

		/* Followed by the page table itself */
		/** memcpy(*buffer, pgd_page, sizeof(u64) * KBASE_MMU_PAGE_ENTRIES); */
		memcpy(*buffer, (void *)pgd_page, sizeof(u64) * KBASE_MMU_PAGE_ENTRIES);
