...
/****** jin: additional headers */
#include <linux/kernel.h>		// jin: for using dump stack
				...
				int js)
{
	...
	if ((core_req & (BASE_JD_REQ_FS | BASE_JD_REQ_CS | BASE_JD_REQ_T)) ==
		...
		// jin: In case of G71, shader_availe = 0xff, debug_core_mask = 0xff
   /**      EE(" -- shaders_avail: %llx, debug_core_mask: %llx affinity: %llx",  */
			/** kbdev->pm.backend.shaders_avail, kbdev->pm.debug_core_mask[js], affinity); */
				...
				int js)
{
	...
	VV("[jin] jc : 0x%llx", katom->jc);
	VV("[jin] jc_low : 0x%08x", (u32) katom->jc & 0xFFFFFFFF);
	VV("[jin] jc_hi : 0x%08llx", katom->jc >> 32); 
	...
	/** EE("[Submit][jc: %llx] js: %llx, cfg: %lx nr_extres: %u, core_req: 0x%x", jc_head, js, cfg, katom->nr_extres, katom->core_req); */
	...
	/** EE("[jin] JS: Submitting atom %p from ctx %p to js[%d] with head=0x%llx",	 */
				/** katom, kctx, js, jc_head); */
	VV("[jin] jc: %llx", katom->jc);
	VV("[jin] nr_extres: %u", katom->nr_extres);
	VV("[jin] core_req: 0x%x", katom->core_req);
	if (katom->softjob_data != NULL)
		VV("[jin] softjob_dat : %p", katom->softjob_data);
	/***************** jin *******************/
/**     int *tmp = (int *) katom->softjob_data;
  *
  *     for (j = 0; j < 10; j++)
  *         dev_info(kbdev->dev, "%3d", tmp[j]);
  *     dev_info(kbdev->dev, "\n"); */
	/******************************************/
	...
	JJ("start job_hw_submit (JS_COMMAND_START). slot_nr : %d", katom->slot_nr);

#if (defined(CONFIG_GR_MEASURE_TIME) && CONFIG_GR_MEASURE_TIME & GR_JOB_EXEC_TIME)
	k2_measure("job_submit_hw_start");
#endif
	/** msleep(1); */
	// jin: add delay
	/** udelay(100); */
	...
	II("job slot (js) : %x", js);
	II("(js) << 7 : %x", js << 7);
	II("kbdev->reg : %p", kbdev->reg);

	II("[jin] End of JS_COMMAND_START");
}
...
static void kbasep_reset_timeout_worker(struct work_struct *data)
{
	...
	spin_lock_irqsave(&kbdev->hwaccess_lock, flags);
	/** EE("[%s] kbase_ctx_sched_restore_all_as"); */
