...
static void kbase_region_tracker_insert(struct kbase_va_region *new_reg)
{
	...
	while (*link) {
		...
		/** DD("old_reg->start_pfn : %lx, start_pfn : %lx", old_reg->start_pfn, start_pfn); */
		...
		struct rb_root *rbtree, u64 start_pfn, size_t nr_pages)
{
   ...
   /**      EE("tmp_start_pfn : %lx, tmp_end_pfn : %lx", tmp_start_pfn, tmp_end_pfn); */
		/** EE("start_pfn : %lx, end_pfn : %lx", start_pfn, end_pfn); */
		...
		struct kbase_va_region *at_reg, u64 start_pfn, size_t nr_pages)
{
	...
	if (at_reg->start_pfn == start_pfn && at_reg->nr_pages == nr_pages) {
		DD("----- Regions are a whole use, so swap and delete old one");
	...
	/* New region replaces the start of the old one, so insert before. */
		...
		//    new_reg    at_reg
		// |----------|----------|
		DD("----- New region replaces the start of the old one, so insert before");
	...
	/* New region replaces the end of the old one, so insert after. */
		...
		DD("----- New region replaces the end of the old one, so insert after");
		at_reg->nr_pages -= nr_pages;
	...
	/* New region splits the old one, so insert and create new */
		...
		// jin
		//  front_reg    new_reg    at_reg
		// |----------|----------|----------|

		DD("----- New region splits the old one, so insert and create new");

		new_front_reg = kbase_alloc_free_region(reg_rbtree,
				at_reg->start_pfn,	// jin: start_pfn
				start_pfn - at_reg->start_pfn,	// jin: nr_pages
   ...
   /**          EE("[%s] new_front_reg | start_pfn : %lx , nr_pages : %lx", \ */
				/** __func__, new_front_reg->start_pfn, new_front_reg->nr_pages); */
			kbase_region_tracker_insert(new_reg);
   /**          EE("[%s] new_reg | start_pfn : %lx , nr_pages : %lx", \ */
				/** __func__, new_reg->start_pfn, new_reg->nr_pages); */
		} else {
			err = -ENOMEM;
		}
	}
		...
		u64 addr, size_t nr_pages, size_t align)
{
	 ...
	 * which *must* be free.
	 */
	VV("[jin] kbase_add_va_region_rbtree : addr : 0x%llx", addr);
	VV("[jin] kbase_add_va_region_rbtree : gpu_pfn : 0x%llx", gpu_pfn);
	if (gpu_pfn) {
		DD("[jin] Path 1: Map a specific address. Find the enclosing region, which must be free");
		KBASE_DEBUG_ASSERT(!(gpu_pfn & (align - 1)));

		// jin: find the region enclosing range from rbtree
		...
		DD("[jin] Path 2: Map any free address which meets the requirements");
...
int kbase_region_tracker_init(struct kbase_context *kctx)
{
	...
	// jin : pfn (in 4KB page units)
	/** EE("gpu_va_end : %lx = %lx (same_va_end) + %lx (custom_va_size)", \ */
		/** kctx->gpu_va_end, kctx->same_va_end, custom_va_size); */
		...
		u8 max_allocations, u8 trim_level, int group_id)
{
	...
	KBASE_TRACE_ADD(kctx->kbdev, JIN_KBASE_IOCTL_DONE, NULL, NULL, 0u, 0);
	return err;
}
...
{
	II("run kbase free alloced region. start_pfn: %llx, nr_pages: %lu", reg->start_pfn, reg->nr_pages);
...
int kbase_gpu_mmap(struct kbase_context *kctx, struct kbase_va_region *reg, u64 addr, size_t nr_pages, size_t align)
{
	...
	if (reg->gpu_alloc->type == KBASE_MEM_TYPE_ALIAS) {
		...
		EE("------------ gpu_alloc type : KBASE_MEM_TYPE_ALIAS");
		...
		// jin: fall here
		/** EE("gpu alloc type is not ALIAS"); */
		/** EE("region->start_pfn : %lx", reg->start_pfn); */
		WW("invoke kbase_mmu_insert_pages [nents : %lu]", reg->gpu_alloc->nents);
		err = kbase_mmu_insert_pages(kctx->kbdev,
				&kctx->mmu,
				reg->start_pfn,		// jin: GPU virtual pfn
				kbase_get_gpu_phy_pages(reg),	// jin: phys
	...
	if (reg->flags & KBASE_REG_IMPORT_PAD &&
		...
		//jin: not here
		/** EE(" ------- flags is KBASE_REG_IMPORT_PAD"); */
		...
		struct basep_syncset *sset, enum kbase_sync_type sync_fn)
{
	...
	VV("[jin] cpu_pa : %p, gpu_pa : %p", *cpu_pa, *gpu_pa);
	...
	WW("cpu_pa_start: 0x%llx cpu_pa_offset: 0x%llx", as_phys_addr_t(cpu_pa[0]), as_phys_addr_t(cpu_pa[page_off]));
	...
	/** jin: 	here is the point that can recognize in/output address space.
	  *         input: should be synced from CPU cache when userspace copies data into mem
	  					as will be used by GPU later
	  *         ouput: will be synced from GPU cache when userspace touches data. 
	  */
#ifdef CONFIG_GR
	if (reg->cpu_alloc == reg->gpu_alloc) {
		void *mapped_page = kmap(as_page(cpu_pa[page_off + i]));

		gr_as_add_sync(kctx->gr_ctx, sset, (page_off << PAGE_SHIFT) + offset);
		JJ("[%d] reg_start_pfn: %llx, fault_addr: %llx, cpu_pa: %llx offset: %llu, page count: %llu page_off: %llu size: %lu", \
			sync_fn, reg->start_pfn, sset->user_addr, as_phys_addr_t(cpu_pa[page_off]), offset, page_count, page_off, size);
		hex_dump(mapped_page + offset, min(size, (size_t) 128));

		kunmap(as_page(cpu_pa[page_off]));
	} else {
		/** TODO: remain this for later as we currently assume that cpu vaddr = gpu vaddr */
	}
#endif
...
int kbase_sync_now(struct kbase_context *kctx, struct basep_syncset *sset)
{
	...
	case BASE_SYNCSET_OP_MSYNC:
		VV("[jin] [sync] sync from cpu to memory");
		err = kbase_do_syncset(kctx, sset, KBASE_SYNC_TO_DEVICE);
		break;

	case BASE_SYNCSET_OP_CSYNC:
		VV("[jin] [sync] sync from memory to cpu");
...
#ifdef CONFIG_GR
	WW("start pfn: %llx, nr_pages: %lu", reg->start_pfn, reg->nr_pages);
	/** gr_dump_vm_region_memory(kctx->gr_ctx, reg); */
#endif
		...
		struct kbase_va_region *reg, unsigned long flags)
{
	...
	if (flags & BASE_MEM_GROW_ON_GPF) {
		/** VV("[jin] BASE_MEM_GROW_ON_GPF");  */
		reg->flags |= KBASE_REG_PF_GROW;
		}

	if (flags & BASE_MEM_PROT_CPU_WR) {
		/** VV("[jin] BASE_MEM_PROT_CPU_WR"); */
		reg->flags |= KBASE_REG_CPU_WR;
		}

	if (flags & BASE_MEM_PROT_CPU_RD) {
		/** VV("[jin] BASE_MEM_PROT_CPU_RD"); */
		reg->flags |= KBASE_REG_CPU_RD;
		}

	if (flags & BASE_MEM_PROT_GPU_WR) {
		/** VV("[jin] BASE_MEM_PROT_GPU_WR"); */
		reg->flags |= KBASE_REG_GPU_WR;
		}

	if (flags & BASE_MEM_PROT_GPU_RD) {
		/** VV("[jin] BASE_MEM_PROT_GPU_RD"); */
		reg->flags |= KBASE_REG_GPU_RD;
		}

	if (0 == (flags & BASE_MEM_PROT_GPU_EX)) {
		/** VV("[jin] BASE_MEM_PROT_CPU_EX"); */
...
int kbase_alloc_phy_pages(struct kbase_va_region *reg, size_t vsize, size_t size)
{
	...
	if (reg->cpu_alloc != reg->gpu_alloc) {
		VV("[jin] reg->cpu_alloc != reg->gpu_alloc");
