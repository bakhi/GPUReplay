...
int kbase_pm_runtime_init(struct kbase_device *kbdev)
{
	...
	if (callbacks) {
		...
		kbdev->pm.backend.callback_soft_reset =
					callbacks->soft_reset_callback;		// jin: Optional callback for software reset, which is not implemented
...
void kbase_pm_register_access_enable(struct kbase_device *kbdev)
{
	...
	VV("[jin] kbase_pm_register_access_enable");
	if (callbacks)
...
void kbase_pm_register_access_disable(struct kbase_device *kbdev)
{
	...
	VV("[jin] kbase_pm_register_access_disable");
	if (callbacks)
...
void kbase_pm_do_poweron(struct kbase_device *kbdev, bool is_resume)
{
	...
	if (!is_resume) {
		...
		/** EE("[PM_UPDATE_STATE] kbase_pm_do_poweron"); */
...
static void kbase_pm_gpu_poweroff_wait_wq(struct work_struct *data)
{
	...
	if (backend->poweron_required) {
		...
		/** EE("[PM_UPDATE_STATE] kbase_pm_gpu_poweroff_wait_wq"); */
		kbase_pm_update_state(kbdev);
...
static void kbase_pm_gpu_clock_control_worker(struct work_struct *data)
{
	...
	spin_lock_irqsave(&kbdev->hwaccess_lock, flags);
	/** EE("[PM_UPDATE_STATE] kbase_pm_gpu_clock_control_worker"); */
	kbase_pm_update_state(kbdev);
	spin_unlock_irqrestore(&kbdev->hwaccess_lock, flags);
}
...
static void kbase_pm_hwcnt_disable_worker(struct work_struct *data)
{
	...
	if (do_disable) {
		...
		/** EE("[PM_UPDATE_STATE] kbase_pm_hwcnt_disable_worker"); */
		kbase_pm_update_state(kbdev);
		kbase_backend_slot_update(kbdev);
	} else {
...
void kbase_pm_do_poweroff(struct kbase_device *kbdev)
{
	 ...
	 */
	/** EE("[PM_UPDATE_STATE] kbase_pm_do_poweroff"); */
	kbase_pm_update_state(kbdev);
		...
		unsigned int flags)
{
	...
	VV("[jin] [%s] kbase_pm_do_poweron", __func__);
...
void kbase_hwaccess_pm_halt(struct kbase_device *kbdev)
{
	...
	mutex_lock(&kbdev->pm.lock);
	dev_info(kbdev->dev, "[%s] kbase_pm_do_poweroff\n", __func__);	// jin
	kbase_pm_do_poweroff(kbdev);
	dev_info(kbdev->dev, "[%s] kbase_pm_do_poweroff done\n", __func__);	// jin
	mutex_unlock(&kbdev->pm.lock);
}
...
void kbase_pm_power_changed(struct kbase_device *kbdev)
{
	...
	spin_lock_irqsave(&kbdev->hwaccess_lock, flags);
	/** EE("[PM_UPDATE_STATE] kbase_pm_power_changed"); */
	kbase_pm_update_state(kbdev);
...
void kbase_hwaccess_pm_suspend(struct kbase_device *kbdev)
{
	...
	II("[jin] kbase_pm_do_poweroff");	// jin
	k2_measure("pm_do_poweroff_start");
#endif
	kbase_pm_do_poweroff(kbdev);
...
#if (defined(CONFIG_GR_MEASURE_TIME) && CONFIG_GR_MEASURE_TIME & GR_PM_TIME)
	II("[jin] kbase_pm_do_poweroff done");
	k2_measure("pm_do_poweroff_done");
#endif
