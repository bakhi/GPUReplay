...
static int kbase_gpu_protected_mode_enter(struct kbase_device *kbdev)
{
	...
	if (kbdev->protected_ops) {
		...
		dev_info(kbdev->dev, "[jin] Switch GPU to protected mode");
	...
	case KBASE_ATOM_ENTER_PROTECTED_FINISHED:
		if (kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_TGOX_R1_1234)) {
			...
			if (kbdev->pm.backend.l2_state == KBASE_L2_ON) {
				...
				dev_info(kbdev->dev, "[%s][%u] kbase_jm_protected_entry", __func__, __LINE__);	// jin
...
void kbase_backend_slot_update(struct kbase_device *kbdev)
{
	...
	// jin: actual nr job slots = 3
	// each rb slot has 2 entries and maximum 2 atoms can be stored
	/** EE("num_job_slots: %u", kbdev->gpu_props.num_job_slots);  */
	for (js = 0; js < kbdev->gpu_props.num_job_slots; js++) {
   ...
   /**      if (katom[0]) */
		...
		for (idx = 0; idx < SLOT_RB_SIZE; idx++) {
			bool cores_ready;
			int ret;
//			u8 atom_number = kbase_jd_atom_id(katom[idx]->kctx, katom[idx]); 	// jin
			/////// jin
//			EE("atom_number : %u", atom_number);
...
//			EE("idx : %d", idx);		// jin
			...
			// jin: not blocked by dep atom
			case KBASE_ATOM_GPU_RB_WAITING_BLOCKED:
				/** EE("KBASE_ATOM_GPU_RB_WAITING_BLOCKED"); */
				if (katom[idx]->atom_flags &
			...
			case KBASE_ATOM_GPU_RB_WAITING_FOR_CORE_AVAILABLE:
				...
				/* fallthrough */
			case KBASE_ATOM_GPU_RB_READY:
				/** k2_measure("KBASE_ATOM_GPU_RB_READY"); */
				/** EE("serialize_jobs : %x", kbdev->serialize_jobs); */
				/** EE("KBASE_ATOM_GPU_RB_READY"); */
				...
				/* fallthrough */
			case KBASE_ATOM_GPU_RB_SUBMITTED:
				/** k2_measure("KBASE_ATOM_GPU_RB_SUBMITTED"); */
				/** EE("KBASE_ATOM_GPU_RB_SUBMITTED"); */
				/* Atom submitted to HW, nothing else to do */
				break;

			case KBASE_ATOM_GPU_RB_RETURN_TO_JS:
				/** k2_measure("KBASE_ATOM_GPU_RB_RETURN_TO_JS"); */
				/** EE("KBASE_ATOM_GPU_RB_RETURN_TO_JS"); */
				/* Only return if head atom or previous atom
				 * already removed - as atoms must be returned
				 * in order */
				if (idx == 0 || katom[0]->gpu_rb_state ==
...
void kbase_backend_run_atom(struct kbase_device *kbdev,
				struct kbase_jd_atom *katom)
{
	VV("[jin] kbase_backend_run_atom");	// jin
	lockdep_assert_held(&kbdev->hwaccess_lock);
	// jin : enqueue an atom in the HW access ringbuffer
	/** k2_measure("kbase_gpu_enqueue_atom"); */
...
#ifdef CONFIG_GR
   /**  EE("---------- gr_print_as invoked ------------"); */
	/** gr_print_as(kctx->gr_ctx); */
#endif
	...
	/** EE("[PM_UPDATE_STATE] kbase_gpu_complete_hw"); */
