	...
	u32 tgid)
{
	...
	const u32 tid = task_pid_nr(current);
	const size_t msg_size = sizeof(msg_id) + sizeof(u64) + sizeof(tid)
		+ sizeof(ctx)
		+ sizeof(ctx_nr)
		+ sizeof(tgid)
		;
	...
	pos = kbasep_serialize_bytes(buffer, pos, &tid, sizeof(tid));
	...
	const char *attrib_match_list)
{
	...
	/** printk("[jin] -------------------- msg_size: %lu s0 size : %lu", msg_size, s0); */
	...
	u32 event)
{
   ...
   /**  printk("pos : %d\n", pos); */
	/** printk("pos : %d, value %u", 0, buffer[0]); */
	/** printk("pos : %d, value %lu", 4, *(u64*)&buffer[4]); */
	/** printk("pos : %d, value %x", 12, *(u64*)&buffer[12]); */
	/** for (i = 20; i < pos; i += 4) */
	/**     printk("pos : %d, value %u", i, buffer[i]); */
	/** printk("\n"); */
